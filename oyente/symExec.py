...
def print_cfg():
    global cfg_vis
    cfg_vis = graphviz.Digraph('G',filename='/home/ziyue/researchlib/oyente/cfg_vis/%s'%g_src_map.parent_filename,node_attr={'shape': 'record', 'height': '.1'})
    cfg_vis.attr(compound='true')
    for block in vertices.values():
        branch_expression = ''
        if hasattr(block,'branch_expression'):
            branch_expression = block.branch_expression
        node_str = r'{BLOCK %d%s %s'%(block.start,r'\n',branch_expression)+r'|{'
        for item in block.instructions:
            node_str += item + r'\n'
        node_str += r'}}'
        cfg_vis.node(str(block.start),node_str)
        #block.display()
    for node in edges:
        for s_node in edges[node]: #s_node: successor
            cfg_vis.edge(str(node),str(s_node))
    cfg_vis.render(view=False)
    #log.debug(str(edges))
